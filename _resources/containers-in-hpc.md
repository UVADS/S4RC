---
layout: page
title: "Tutorial: Use of Containers for Interactive JupyterLab and Batch Computing in HPC Environments"
...
- Have a basic understanding of software containers. See <a href="https://uvads.github.io/container-basics/">Container Basics</a> and <a href="{{ site.baseurl }}/resources/container-registries/">Container Registries</a>

<div style="padding: 1em 1.5em; margin: 1.5em 0; border-left: 4px solid #0c5460; background-color: #e7f3f5; border-radius: 4px;" markdown="1">
It is assumed that you have access to an HPC system. HPC access and user accounts are typically tied to an **allocation**, which is a grant of compute resources that allows you to submit and run jobs on the HPC cluster. At UVA, faculty can request allocations through the [Research Computing](https://www.rc.virginia.edu/userinfo/hpc/). Postdocs, staff and students can be sponsored through a faculty allocation.
</div>
...
![New PyTorch 2.9.1 kernel in JupyterLab]({{ site.baseurl }}/assets/images/containers-in-hpc/JupyterLab-newkernel.png)

### 5. A Notebook Example

For this tutorial we will use a [pytorch-example.ipynb]({{ site.baseurl }}/assets/code/containers-in-hpc/pytorch-example.ipynb) notebook to perform image classification on the MNIST dataset. It's been adapted from example code in the [PyTorch example](https://github.com/pytorch/examples/) repository. 
...
```bash
mkdir -p /scratch/$USER/jobrun      # create new directory in scratch directory
cd /scratch/$USER/jobrun            # change into that new directory
sbatch ~/pytorch-example.sh         # submit the job script; it is in our ~/ home directory
```
...
WORKDIR /opt/my_app # customize path
...
# Copy the local Git repository, includes requirements.txt
COPY . ./
...
ENTRYPOINT ["/usr/bin/tini", "-g", "--", "/opt/my_app/entrypoint.sh"]
```
